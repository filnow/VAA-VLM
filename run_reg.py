...
class ModelEvaluator:
    ...
    def _create_prompts_for_image(self, base64_image: str, emotion_definitions: dict) -> List[List[Dict]]:
        """Creates a list of conversation prompts, one for each emotion."""
        conversations = []
        ...
        for emotion_name, definition in emotion_definitions.items():
            ...
            user_text = self.prompt_templates['user_text_template'].format(
    ...
    def rate_emotions(self, dataset, emotion_definitions: dict) -> pd.DataFrame:
        ...
        for item in tqdm(dataset, desc="Processing images"):
            ...
            prompts = self._create_prompts_for_image(base64_image, emotion_definitions)
            outputs = self.llm.chat(prompts, sampling_params=self.sampling_params, use_tqdm=False)

            row_data = {'image': item['filename']}
            for i, output in enumerate(outputs):
                emotion_name = emotions_to_rate[i]
                scores = []
                for completion in output.outputs:
                    text = completion.text.strip()
                    if text.isdigit() and 1 <= int(text) <= self.cfg.dataset.scale:
                ...
                if not scores: 
                    ...
                    std_score = round(torch.std(torch.tensor(scores)).item(), 4) if len(scores) > 1 else 0.0
...
@hydra.main(version_base=None, config_path="configs", config_name="config-reg")
